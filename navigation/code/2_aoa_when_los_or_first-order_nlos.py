...
def main():
    ...
    # TX pos
    # please enter the name of map, testing tx number, and TX location
    map_name = <'MAP_NAME'>		# for example: map_name = 'Adrian'
    tx_name = <'TX_NUMBER'>		# for example: tx_name = '10'
    x_y = <[X_TX, Y_TX]>		# for example: x_y = [9.3,20.45] #10
    ...
    # ## Akshaj Added: Load wireless data
    # from utils.signal_source_data import Mdata, Mlinkstate
    # aoa = Mdata()
    # linkstate = Mlinkstate()
    # ##
    # Mingsheng add
    aoa_angle = np.loadtxt('./data/wireless_data/txt/'+map_name+'/'+map_name+'_Tx_'+tx_name+'_aoa.txt')
    link_state = np.loadtxt('./data/wireless_data/txt/'+map_name+'/'+map_name+'_Tx_'+tx_name+'_linkstate.txt')
    ...
    for ep_num in range(num_episodes):
        for step in range(args.max_episode_length):
            ...
            x_path.append(start_j[0])
            y_path.append(start_j[1])
            xy_path.append([start_j[0], start_j[1]])
            # check if reach the TX
            # print(int(abs(start_j[0]-x_tx) + abs(start_j[1]-y_tx)))
            if go_SLAM_flag == False:
                if xy_path.count([start_j[0], start_j[1]]) >= 15:
                    xy_path.remove([start_j[0], start_j[1]])
                    go_SLAM = GO_SLAMINT+3
                    go_SLAM_flag = True
                    print(go_SLAM)
                    GO_SLAMINT = go_SLAM
                    print(GO_SLAMINT)
            if total_num_steps == 999:
                np.savetxt(dump_dir+'path', np.array([x_path, y_path], dtype=int))
                np.savetxt(dump_dir+'TX_pos', np.array([x_tx, y_tx], dtype=int))

            if int(abs(start_j[0]-x_tx) + abs(start_j[1]-y_tx)) <= 6:
                print(x_path)
                print(len(x_path))
                np.savetxt(dump_dir+'path', np.array([x_path, y_path], dtype=int))
                np.savetxt(dump_dir+'TX_pos', np.array([x_tx, y_tx], dtype=int))
            ...
            # print(f'l_step:{l_step} - planner_pose_inputs {start_cj}, {start_rj}')
            # ------------------------------------------------------------------
            # Global Policy
            if l_step == args.num_local_steps - 1:
                ...
                g_masks = torch.ones(num_scenes).float().to(device)
            # ------------------------------------------------------------------
                ## Akshaj Added: Set global goal for locating Tx  
                if global_goals_record != None and \
                int(abs(global_goals_record[0]-x_tx) + abs(global_goals_record[1]-y_tx)) <= 6:
                    print(x_path)
                    print(len(x_path))
                    np.savetxt(dump_dir+'path', np.array([x_path, y_path], dtype=int))
                    np.savetxt(dump_dir+'TX_pos', np.array([x_tx, y_tx], dtype=int))
                ...
                for e, action in enumerate(cpu_actions):
                    if go_SLAM_flag and go_SLAM >0:
                        go_SLAM -= 1
                        if go_SLAM == 0:
                            go_SLAM_flag = False
                    else:
                        start_cj, start_rj, start_oj, gx1j, gx2j, gy1j, gy2j = planner_pose_inputs[e]
                        o0, o1, o2 = origins[e]
                        start_j = int(start_cj * 100.0/args.map_resolution), int(start_rj * 100.0/args.map_resolution)
                        # start_j = int((start_cj-o0) * 100.0/args.map_resolution), int((start_rj-o1) * 100.0/args.map_resolution)
                        o0, o1, o2 = origins[e]
                        o0 = o0/0.05
                        o1 = o1/0.05
                        if start_j[1] >= 480:
                            start_j = list(start_j)
                            start_j[1] = 479
                        if start_j[0] >= 480:
                            start_j = list(start_j)
                            start_j[0] = 479
                        ...
                        aoa2 = aoa_angle[start_j[1], start_j[0]]
                        ls = link_state[start_j[1], start_j[0]]
                        if np.isnan(ls):
                            ls = 0
                        else:
                            ls = int(ls)
                        # print('step: ', step, 'Linkstate is: ', ls)
                        if ls == 1 or ls == 2:
                            # print("Following AoA")
                            # print(aoa2)  
                            ...
                            x_gc = start_j[0] + 12*x_lc
                            y_gc = start_j[1] + 12*y_lc
            
                            global_goals[e] = [int(((y_gc) - o1)), int((x_gc) - o0)]
                            global_goals_record = [x_gc, y_gc]

                        # if ls == 0 or ls == 3:
                        #     print("Following NeuralSLAM")  

                            # x_gc = global_goals[e][1] + o0
                            # y_gc = global_goals[e][0] + o1
                        # current postion
                        # print(f'{start_j[0]},{start_j[1] }')
                        # a0 = start_j[0]
                        # a1 = start_j[1] 
